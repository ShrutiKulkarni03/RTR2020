...
void Display(void)
{
	...
	rotateXMatrix = rotate(90.0f, 1.0f, 0.0f, 0.0f);  //sun axis change
	modelViewMatrix = modelViewMatrix * rotateXMatrix;
	
	modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;  //pre-multiplication of matrices
	glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, modelViewProjectionMatrix);
	
	glVertexAttrib3f(SPK_ATTRIBUTE_COLOR, 1.0f, 0.5f, 0.0f);
	...
	rotateYMatrix = rotate((GLfloat)year, 0.0f, 1.0f, 0.0f);
	translateMatrix = translate(2.5f, 0.0f, 0.0f);
	scaleMatrix = scale(0.3f, 0.3f, 0.3f);
	rotateXMatrix = rotate(90.0f, 1.0f, 0.0f, 0.0f);   //earth axis change
	modelViewMatrix = modelViewMatrix * rotateYMatrix * translateMatrix * scaleMatrix * rotateXMatrix;

	Push(pStack, modelViewMatrix);

	rotateZMatrix = rotate((GLfloat)day, 0.0f, 0.0f, 1.0f);
	modelViewMatrix = modelViewMatrix * rotateZMatrix;

	modelViewProjectionMatrix = perspectiveProjectionMatrix * modelViewMatrix;  //pre-multiplication of matrices
	glUniformMatrix4fv(mvpMatrixUniform, 1, GL_FALSE, modelViewProjectionMatrix);

	glVertexAttrib3f(SPK_ATTRIBUTE_COLOR, 0.4f, 0.9f, 1.0f);
	...
	rotateZMatrix = rotate((GLfloat)day, 0.0f, 0.0f, 1.0f);
	translateMatrix = translate(3.0f, 0.0f, 0.0f);
	scaleMatrix = scale(0.5f, 0.5f, 0.5f);

	modelViewMatrix = modelViewMatrix * rotateZMatrix * translateMatrix * scaleMatrix;

	rotateZMatrix = rotate((GLfloat)hour, 0.0f, 0.0f, 1.0f);
	modelViewMatrix = modelViewMatrix * rotateZMatrix;
