...
COLOR SunColor = { 1.0f, 0.42f, 0.03f };
POSITION SunTranslate = { 0.0f, -2.0f, -25.7 };


//SKY

COLOR SkyTop = { 0.003f, 0.035f, 0.078f };
COLOR SkyMid = { 0.59f, 0.67f, 0.89f };
COLOR SkyBottom = { 1.0f, 0.56f, 0.11f };
GLfloat Pt1Alpha = 0.0f;
GLfloat Pt2Alpha = 0.0f;
GLfloat Pt3Alpha = 0.0f;
...
GLfloat introTypoAlpha = 0.0f;
GLfloat endCreditsAlpha = 1.0f;
GLfloat TYCreditsAlpha = 0.0f;
...
GLfloat SunColorAnimateSpeed = 0.0002f;
GLfloat SkyColorAnimateSpeed = 0.0004f;
...
GLfloat TimerSpeed = 0.01f;
GLfloat TypoAlphaSpeed = 0.005;
...
GLuint wall_texture;
GLuint front_wall_door_closed_texture;
GLuint front_wall_door_opened_texture;
...
void Sky(void)
{
	...
	//{
	//	GLfloat StarsXn = 40 * ((GLfloat)rand() / (GLfloat)RAND_MAX) ;
	//	GLfloat StarsYn = 40 * ((GLfloat)rand() / (GLfloat)RAND_MAX) ;
	//	//GLfloat StarsXp = 40 * ((GLfloat)rand() / (GLfloat)RAND_MAX);
	//	//GLfloat StarsYp = 40 * ((GLfloat)rand() / (GLfloat)RAND_MAX);

	//	//glVertex3f(StarsXp, StarsYp, -25.5f);
	//	glVertex3f(StarsXn, StarsYn, -25.5f);
...
void Stars(void)
{
	...
	glColor4f(1.0f, 1.0f, 1.0f, Pt2Alpha);
	...
	glColor4f(1.0f, 1.0f, 1.0f, Pt3Alpha);
...
void House(void)
{
	...
	if (isDoorOpen == true)
	{
		front_wall = front_wall_door_opened_texture;
	}
	else
	{
		front_wall = front_wall_door_closed_texture;
	}

	

	glBindTexture(GL_TEXTURE_2D, front_wall);
...
void Scene0(void)
{
	...
	if (isScene0 == true)
	{
		introTypoAlpha += TypoAlphaSpeed;
		if (introTypoAlpha >= 1.0f)
		{
			introTypoAlpha = 1.0f;
		}
	}
	...
	if (Timer >= 0.5f)
	{
		introTypoAlpha -= 0.01f;
		if (introTypoAlpha <= 0.0f)
		{
			introTypoAlpha = 0.0f;
		}
...
void Scene1(void)
{
	...
	if (isScene1 == true)
	{
		...
		Bird2Translate.X -= BirdsTranslateSpeed - 0.01f;
		if (Bird2Translate.X <= 6.5f)
		{
			Bird2Translate.X = 6.5f;
		}

		Bird2Translate.Y -= BirdsTranslateSpeed - 0.01f;
		if (Bird2Translate.Y <= -0.5f)
		{
			Bird2Translate.Y = -0.5f;
		}

		Bird2Translate.Z -= BirdsTranslateSpeed - 0.01f;
	...
	if (isScene2 == true)
	{
		...
		case 5:
			...
			if (isBird1Still == false)
			{
				...
				Bird1RotateAngle += BirdsRotateSpeed + 0.2f;
				...
				Bird1Translate.X -= BirdsTranslateSpeed + 0.01f;
				...
				Bird1Translate.Y -= BirdsTranslateSpeed - 0.003f;
			...
			if (isBird2Still == false)
			{
				...
				Bird2Translate.X -= BirdsTranslateSpeed + 0.021f;
				...
				Bird2Translate.Z += BirdsTranslateSpeed + 0.012f;
...
void Scene3(void)
{
	...
	if (isScene3 == true)
	{
		...
		case 0:
			...
			case 0:
				Girl2RtShoulderRotate -= GirlsHandsRotateSpeed - 0.2f;
			...
			case 0:
				Girl2LtShoulderRotate += GirlsHandsRotateSpeed - 0.2f;
...
void Scene4(void)
{
	...
	if (isScene4 == true)
	{
		...
		if (isBird4Still == true)
		{
			...
			Bird1RotateAngle += BirdsRotateSpeed + 3.0f;
			if (Bird1RotateAngle >= 200.0f)
			{
				Bird1RotateAngle = 200.0f;
...
void Scene5(void)
{
	...
	if (isScene5 == true)
	{
		...
		if (isBird1Still == false)
		{
			...
			Bird1Translate.X += BirdsTranslateSpeed + 0.007f;
			...
			Bird1Translate.Z -= BirdsTranslateSpeed + 0.007f;
		...
		if (isBird1Still == true)
		{
			...
			/*Bird1RotateAngle -= BirdsRotateSpeed + 2.0f;
		...
		if (isBird2Still == false)
		{
			...
			Bird2Translate.X += BirdsTranslateSpeed + 0.009;
		...
		case 3:
			...
			Girl2Translate.X += GirlsTranslateSpeed - 0.001f;
			if (Girl2Translate.X >= -2.5f)
			{
				Girl2Translate.X = -2.5f;
				isDoorOpen = false;
				Girl2Movements++;
				//isGirl2Still = true;
			}
			...
			Girl2Translate.Z += GirlsTranslateSpeed;
			if (Girl2Translate.Z >= 5.5f)
			{
				Girl2Translate.Z = 5.5f;
...
void Scene6(void)
{
	...
	if (Timer >= 6.5f)
	{
		endCreditsAlpha -= TypoAlphaSpeed * 3;

		if (endCreditsAlpha <= 0.0f)
		{
			endCreditsAlpha = 0.0f;
		}
	}
	...
	if (Timer >= 7.5f)
	{
		TYCreditsAlpha += TypoAlphaSpeed * 1.5;

		if (TYCreditsAlpha >= 1.0f)
		{
			TYCreditsAlpha = 1.0f;
		}
	}
	...
	if (Timer >= 10.0f)
	{
		TYCreditsAlpha -= TypoAlphaSpeed * 3;

		if (TYCreditsAlpha <= 0.0f)
		{
			TYCreditsAlpha = 0.0f;
		}
	}
	...
	if (Timer >= 10.5f)
	{
		FadeInQuadAlpha += FadeInQuadAlphaSpeed * 1.5;
		if (FadeInQuadAlpha >= 1.0f)
		{
			FadeInQuadAlpha = 1.0f;
