...
HRESULT Initialize(void)
{
	...
	/*	const float quadVertices[] =
		...
		const float quadTexcoords[] = { 1.0f, 1.0f,        //front
										0.0f, 1.0f,
										1.0f, 0.0f,
										0.0f, 0.0f };
		...
		//texture
		hr = loadD3DTexture(&gpID3D11ShaderResourceView_texture_quad);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
		...
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
			fprintf(gpFile, ("loadD3DTexture() - successful\n\n"));
			fclose(gpFile);
		}
...
HRESULT loadD3DTexture(ID3D11ShaderResourceView** ppID3D11ShaderResourceView)
{
	...
	ID3D11Texture2D* pID3D11Texture2D_checkImageBuffer;

	hr = gpID3D11Device->CreateTexture2D(&d3d11Texture2dDesc, &d3d11SubresourceData, &pID3D11Texture2D_checkImageBuffer);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, "RenderLog.txt", "a+");
		fprintf(gpFile, ("CreateTexture2D() checker failed\n\n"));
		fclose(gpFile);
		Uninitialize();
	}
	else
	{
		fopen_s(&gpFile, "RenderLog.txt", "a+");
		fprintf(gpFile, ("CreateTexture2D() checker successful\n\n"));
		fclose(gpFile);
	}


	hr = gpID3D11Device->CreateShaderResourceView(pID3D11Texture2D_checkImageBuffer, NULL, ppID3D11ShaderResourceView);
	if (FAILED(hr))
	{
		fopen_s(&gpFile, "RenderLog.txt", "a+");
		fprintf(gpFile, ("CreateShaderResourceView() checker failed\n\n"));
		fclose(gpFile);
		Uninitialize();
	}
	else
	{
		fopen_s(&gpFile, "RenderLog.txt", "a+");
		fprintf(gpFile, ("CreateShaderResourceView() checker successful\n\n"));
		fclose(gpFile);
	}
...
void Display(void)
{
	...
	XMMatrixTranslationMatrix = XMMatrixTranslation(0.0f, 0.0f, 5.0f);
	...
	//draw
	gpID3D11DeviceContext->Draw(4, 0);   //0-6, 6-6, 12-6, 18-6, 24-6, 30-6  

	//angular
	quadVertices[0] = 1.0f;
	quadVertices[1] = 1.0f;
	quadVertices[2] = 0.0f;
	quadVertices[3] = 2.41421f;
	quadVertices[4] = 1.0f;
	quadVertices[5] = 1.41421f;
	quadVertices[6] = 1.0f;
	quadVertices[7] = -1.0f;
	quadVertices[8] = 0.0f;
