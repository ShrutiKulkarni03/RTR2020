...
HRESULT Initialize(void)
{
	...
	else
	{
		...
		if (FAILED(hr))
		{
			...
			{
				fopen_s(&gpFile, "RenderLog.txt", "a+");
				fprintf(gpFile, "COM error for geometry shader\n\n");
				fclose(gpFile);
				Uninitialize();
			}
		...
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
			fprintf(gpFile, ("D3DCompile() successful for geometry shader\n\n"));
			fclose(gpFile);
		}

		//create shader
		hr = gpID3D11Device->CreateGeometryShader(pID3DBlob_GeometryShaderCode->GetBufferPointer(), pID3DBlob_GeometryShaderCode->GetBufferSize(), NULL, &gpID3D11GeometryShader);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
			fprintf(gpFile, ("ID3D11Device::CreateGeometryShader() failed\n\n"));
			fclose(gpFile);
			Uninitialize();
		}
		else
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
			fprintf(gpFile, ("ID3D11Device::CreateGeometryShader() successful\n\n"));
			fclose(gpFile);
		}
		...
		inputElementDescTriangle.Format = DXGI_FORMAT_R32G32B32_FLOAT;
...
void Display(void)
{
	...
	XMMatrixTranslationMatrix = XMMatrixTranslation(0.0f, 0.0f, 3.0f);
