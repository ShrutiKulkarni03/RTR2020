...
ID3D11Buffer* gpID3D11Buffer_VertexBuffer_color_quad = NULL;
...
HRESULT Initialize(void)
{
	...
	else
	{
		...
		inputElementDescTriangle[1].SemanticName = "COLOR";
		inputElementDescTriangle[1].SemanticIndex = 0;
		inputElementDescTriangle[1].Format = DXGI_FORMAT_R32G32B32_FLOAT;
		inputElementDescTriangle[1].InputSlot = 1;
		inputElementDescTriangle[1].AlignedByteOffset = 0;
		...
		inputElementDescQuad[1].SemanticName = "COLOR";
		...
		hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_position_triangle);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
		...
		gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_position_triangle, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubResource);
		memcpy(d3d11MappedSubResource.pData, triangleVertices, sizeof(triangleVertices));
		...
		hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_color_triangle);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
			fprintf(gpFile, ("ID3D11Device::CreateBuffer() failed\n\n"));
		...
		//D3D11_MAPPED_SUBRESOURCE d3d11MappedSubResource;
		ZeroMemory(&d3d11MappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
		gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_color_triangle, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubResource);
		memcpy(d3d11MappedSubResource.pData, triangleColors, sizeof(triangleColors));
		...
		//QUAD COLOR
		ZeroMemory((void*)&d3d11BufferDesc, sizeof(D3D11_BUFFER_DESC));
		d3d11BufferDesc.ByteWidth = sizeof(float) * _ARRAYSIZE(quadColors);
		d3d11BufferDesc.BindFlags = D3D11_BIND_VERTEX_BUFFER;   //glBindBuffer();
		d3d11BufferDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
		d3d11BufferDesc.Usage = D3D11_USAGE_DYNAMIC;

		hr = gpID3D11Device->CreateBuffer(&d3d11BufferDesc, NULL, &gpID3D11Buffer_VertexBuffer_color_quad);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
			fprintf(gpFile, ("ID3D11Device::CreateBuffer() failed\n\n"));
		...
		//D3D11_MAPPED_SUBRESOURCE d3d11MappedSubResource;
		ZeroMemory(&d3d11MappedSubResource, sizeof(D3D11_MAPPED_SUBRESOURCE));
		gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBuffer_color_quad, 0, D3D11_MAP_WRITE_DISCARD, 0, &d3d11MappedSubResource);
		memcpy(d3d11MappedSubResource.pData, quadColors, sizeof(quadColors));
		gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBuffer_color_quad, 0);
...
void Display(void)
{
	...
	XMMATRIX XMMatrixRotationMatrix = XMMatrixIdentity();
	XMMATRIX worldMatrix = XMMatrixIdentity();
	XMMATRIX viewMatrix = XMMatrixIdentity();

	XMMatrixTranslationMatrix = XMMatrixTranslation(-2.0f, 0.0f, 6.0f);
	XMMatrixRotationMatrix = XMMatrixRotationY(angleTriangle);
	worldMatrix = XMMatrixRotationMatrix * XMMatrixTranslationMatrix;
	...
	stride = sizeof(float) * 3;
	offset = 0;
	gpID3D11DeviceContext->IASetVertexBuffers(1, 1, &gpID3D11Buffer_VertexBuffer_color_quad, &stride, &offset);
	...
	XMMatrixRotationMatrix = XMMatrixIdentity();
	worldMatrix = XMMatrixIdentity();
	viewMatrix = XMMatrixIdentity();

	XMMatrixTranslationMatrix = XMMatrixTranslation(2.0f, 0.0f, 6.0f);
	XMMatrixRotationMatrix = XMMatrixRotationX(angleQuad);
	worldMatrix = XMMatrixRotationMatrix * XMMatrixTranslationMatrix;
...
void Uninitialize(void)
{
	...
	if (gpID3D11Buffer_VertexBuffer_color_quad)
	{
		gpID3D11Buffer_VertexBuffer_color_quad->Release();
		gpID3D11Buffer_VertexBuffer_color_quad = NULL;
	}
