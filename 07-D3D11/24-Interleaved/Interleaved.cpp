...
struct CBUFFER
{
	...
	unsigned int CKeyPressed;
	unsigned int LKeyPressed;
	unsigned int TKeyPressed;
};
...
HRESULT Initialize(void)
{
	...
	else
	{
		...
		const char* vertexShaderSourceCode =
			...
			"	float3 s = ((float3)normalize(lightPosition - eyePosition));\n" \
		...
		const char* pixelShaderSourceCode =
			...
			"		o_col = input.color; \n" \
			"	else \n" \
			"		o_col = float4(1.0, 1.0, 1.0, 1.0); \n" \
			
			"	if(lKeyPressed == 1) \n" \
			"		o_lit = input.diffuse_light; \n" \
			"	else \n" \
			"		o_lit = float4(1.0, 1.0, 1.0, 1.0); \n" \

			"	if(tKeyPressed == 1) \n" \
			"		o_tex = myTexture2D.Sample(mySamplerState, input.texcoord); \n" \
			"	else \n" \
			"		o_tex = float4(1.0, 1.0, 1.0, 1.0); \n" \

			"	result = o_col * o_lit * o_tex; \n" \
			"	return(result); \n" \
		...
		D3D11_INPUT_ELEMENT_DESC inputElementDescCube[4];
		//ZeroMemory((void*)&inputElementDesc, sizeof(D3D11_INPUT_ELEMENT_DESC));

		inputElementDescCube[0].SemanticName = "POSITION";
		inputElementDescCube[0].SemanticIndex = 0;
		inputElementDescCube[0].Format = DXGI_FORMAT_R32G32B32_FLOAT;
		inputElementDescCube[0].InputSlot = 0;
		inputElementDescCube[0].AlignedByteOffset = 0;
		inputElementDescCube[0].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
		inputElementDescCube[0].InstanceDataStepRate = 0;

		inputElementDescCube[1].SemanticName = "COLOR";
		inputElementDescCube[1].SemanticIndex = 0;
		inputElementDescCube[1].Format = DXGI_FORMAT_R32G32B32_FLOAT;
		inputElementDescCube[1].InputSlot = 1;
		inputElementDescCube[1].AlignedByteOffset = 0;
		inputElementDescCube[1].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
		inputElementDescCube[1].InstanceDataStepRate = 0;

		inputElementDescCube[2].SemanticName = "NORMAL";
		inputElementDescCube[2].SemanticIndex = 0;
		inputElementDescCube[2].Format = DXGI_FORMAT_R32G32B32_FLOAT;
		inputElementDescCube[2].InputSlot = 2;
		inputElementDescCube[2].AlignedByteOffset = 0;
		inputElementDescCube[2].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
		inputElementDescCube[2].InstanceDataStepRate = 0;

		inputElementDescCube[3].SemanticName = "TEXCOORD";
		inputElementDescCube[3].SemanticIndex = 0;
		inputElementDescCube[3].Format = DXGI_FORMAT_R32G32_FLOAT;
		inputElementDescCube[3].InputSlot = 3;
		inputElementDescCube[3].AlignedByteOffset = D3D11_APPEND_ALIGNED_ELEMENT;
		inputElementDescCube[3].InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
		inputElementDescCube[3].InstanceDataStepRate = 0;


		hr = gpID3D11Device->CreateInputLayout(inputElementDescCube, _ARRAYSIZE(inputElementDescCube), pID3DBlob_VertexShaderCode->GetBufferPointer(), pID3DBlob_VertexShaderCode->GetBufferSize(), &gpID3D11InputLayout);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
		...
		//texture
		hr = loadD3DTexture(L"marble.bmp", &gpID3D11ShaderResourceView_texture); //L: Literal (used for wide character)
		if (FAILED(hr))
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
...
void Display(void)
{
	...
	XMMATRIX pMatrix = gPerspectiveProjectionMatrix;
	...
	if (bColor == true)
		constantBuffer.CKeyPressed = 1;
	else
		constantBuffer.CKeyPressed = 0;
	...
	if(bTexture == true)
		constantBuffer.TKeyPressed = 1;
	else
		constantBuffer.TKeyPressed = 0;
