...
HRESULT Initialize(void)
{
	...
	else
	{
		...
		D3D11_INPUT_ELEMENT_DESC inputElementDescQuad;
		//ZeroMemory((void*)&inputElementDesc, sizeof(D3D11_INPUT_ELEMENT_DESC));

		inputElementDescQuad.SemanticName = "POSITION";
		inputElementDescQuad.SemanticIndex = 0;
		inputElementDescQuad.Format = DXGI_FORMAT_R32G32B32_FLOAT;
		inputElementDescQuad.InputSlot = 0;
		inputElementDescQuad.AlignedByteOffset = 0;
		inputElementDescQuad.InputSlotClass = D3D11_INPUT_PER_VERTEX_DATA;
		inputElementDescQuad.InstanceDataStepRate = 0;

		hr = gpID3D11Device->CreateInputLayout(&inputElementDescQuad, 1, pID3DBlob_VertexShaderCode->GetBufferPointer(), pID3DBlob_VertexShaderCode->GetBufferSize(), &gpID3D11InputLayout);
		if (FAILED(hr))
		{
			fopen_s(&gpFile, "RenderLog.txt", "a+");
