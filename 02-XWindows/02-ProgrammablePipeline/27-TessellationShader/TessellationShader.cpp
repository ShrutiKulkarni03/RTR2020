...
void Initialize(void)
{
	...
	glShaderSource(tessellationControlShaderObject, 1, (const GLchar**)&tessellationControlSourceCode, NULL);
	...
	if (shaderCompiledStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			{
				GLsizei written;
				glGetShaderInfoLog(tessellationControlShaderObject, infoLogLength, &written, szBuffer);
	...
	glShaderSource(tessellationEvaluationShaderObject, 1, (const GLchar**)&tessellationEvaluationSourceCode, NULL);
	...
	if (shaderCompiledStatus == GL_FALSE)
	{
		...
		if (infoLogLength > 0)
		{
			...
			{
				GLsizei written;
				glGetShaderInfoLog(tessellationEvaluationShaderObject, infoLogLength, &written, szBuffer);
...
free(szBuffer);
    ...
    //attach tess control shader to shader program
	glAttachShader(shaderProgramObject, tessellationControlShaderObject);

	//attach tess evaluation shader to shader program
	...
	const GLfloat vertices[] = { -1.0f, -1.0f,
								 -0.5f, 1.0f,
								  0.5f, -1.0f,
								  1.0f, 1.0f  };
	...
	glVertexAttribPointer(SPK_ATTRIBUTE_POSITION, 2, GL_FLOAT, GL_FALSE, 0, NULL);
...
void Draw(void)
{
	...
	//glUniform4f(lineColorUniform, 1.0f, 1.0f, 0.0f, 1.0f);
	...
	glDrawArrays(GL_TRIANGLES, 0, 3);glPatchParameteri(GL_PATCH_VERTICES, 4);
