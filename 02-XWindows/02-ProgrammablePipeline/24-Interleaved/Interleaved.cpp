...
void Initialize(void)
{
	...
	//CUBE
	                             //verices              //normals             //texcoords
	const GLfloat cubeVNT[] = {  1.0f, 1.0f, 1.0f,      0.0f, 0.0f, 1.0f,     1.0f, 1.0f,
	...
	glBufferData(GL_ARRAY_BUFFER, 24 * 8 * sizeof(float), cubeVNT, GL_STATIC_DRAW);
	glVertexAttribPointer(SPK_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), 0);
	glEnableVertexAttribArray(SPK_ATTRIBUTE_POSITION);
	glVertexAttribPointer(SPK_ATTRIBUTE_NORMAL, 3, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(3 * sizeof(float)));
	glEnableVertexAttribArray(SPK_ATTRIBUTE_NORMAL);
	glVertexAttribPointer(SPK_ATTRIBUTE_TEXCOORD, 2, GL_FLOAT, GL_FALSE, 8 * sizeof(float), (void*)(6 * sizeof(float)));
